...
test('真实后端完成章节总结、大纲对照、写作和人工审查', async ({ page, request }) => {
  ...
  const targetColumn = page.locator('.outline-column').filter({ has: page.getByRole('heading', { name: '目标大纲' }) });
  const targetItem = targetColumn.locator('textarea').nth(1);
  await expect(targetItem).toHaveValue('李四发现院门暗记');
  await targetItem.fill('李四在院门上发现新的暗记');
  await page.getByRole('button', { name: '新增大纲' }).click();
  await expect(page.getByLabel('第 4 条操作')).toHaveValue('insert');
  await page.getByRole('button', { name: '保存目标大纲' }).click();

  await expect(page.getByRole('heading', { name: '确定写作风格' })).toBeVisible();
  await page.getByRole('button', { name: '确定风格' }).click();
  ...
  expect(state.special_analysis.target_outline).toHaveLength(4);
});
