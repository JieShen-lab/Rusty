...
function PromptPreview({ prompt }: { prompt: PromptTemplate | null }) { if (!prompt) return <div className="workspace-message"><h2>提示词预览</h2><p>请先完成全书归纳。</p></div>; return <div className="prompt-preview"><section><h3>基础规则</h3><pre>{prompt.global_rules || '暂无'}</pre></section><section><h3>识别规则</h3><pre>{prompt.scene_detection_rules || '暂无'}{prompt.scene_rules.map((rule) => `\n\n[${rule.display_name}]\n${rule.detection_prompt}`).join('')}</pre></section><section><h3>改写规则</h3><pre>{prompt.rewrite_rules || '暂无'}{prompt.scene_rules.map((rule) => `\n\n[${rule.display_name}]\n${rule.rewrite_prompt}`).join('')}</pre></section></div>; }
