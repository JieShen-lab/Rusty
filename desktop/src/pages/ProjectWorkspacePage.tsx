...
function SceneRewritePanel(props: {
  ...
  const [plotMaterialId, setPlotMaterialId] = useState('');
  const [sceneMaterialIds, setSceneMaterialIds] = useState('');
  const [insertion, setInsertion] = useState('场景中部');
  ...
  const ids = (value: string) => value.split(/[,\s，]+/).map(Number).filter((item) => Number.isInteger(item) && item > 0);
  ...
  async function start() {
    if (!selected) return;
    await perform(async () => {
      const next = await startSceneWorkflow(selected.id, { mode, user_instruction: instruction, character_ids: ids(characterIds), material_ids: [...ids(plotMaterialId), ...ids(sceneMaterialIds)] });
      setRun(next);
  ...
  async function confirmSkeletonAndPlan() {
    ...
    await perform(async () => {
      ...
      const mappings = mode === 'expansion' ? ids(plotMaterialId).map((materialId) => ({ material_id: materialId, insertion_after_node: insertion, usage_mode: 'required' })) : [];
      const next = await generateSceneWorkflowPlan(currentRun.id, { skeleton_version_id: skeleton.version_id, user_instruction: instruction, character_ids: ids(characterIds), material_mappings: mappings, scene_reference_ids: ids(sceneMaterialIds) });
      setRun(next);
  ...
  async function confirmPlanAndExecute() {
    ...
    await perform(async () => {
      ...
      const result = await executeSceneWorkflow(run.id, { user_instruction: instruction, character_ids: ids(characterIds), material_ids: [...ids(plotMaterialId), ...ids(sceneMaterialIds)] });
      setRun(result);
        ...
        <div className="scene-workflow-grid">
          ...
          <main>
            ...
            <section className="scene-workflow-form">
              ...
              <label><span>角色卡 ID</span><input value={characterIds} onChange={(event) => setCharacterIds(event.target.value)} placeholder="例如 1, 2" /></label>
              {mode === 'expansion' ? <label><span>剧情骨架素材 ID（产生新增事件）</span><input value={plotMaterialId} onChange={(event) => setPlotMaterialId(event.target.value)} /></label> : null}
              <label><span>场景素材 ID（仅写法参考）</span><input value={sceneMaterialIds} onChange={(event) => setSceneMaterialIds(event.target.value)} /></label>
              {mode === 'expansion' ? <label><span>插入位置</span><input value={insertion} onChange={(event) => setInsertion(event.target.value)} /></label> : null}
            ...
            {history.length ? <section><h3>版本历史（原文不被覆盖）</h3>{history.map((item, index) => <details key={String(item.id ?? index)}><summary>版本 {String(item.version ?? index + 1)}</summary><pre>{String(item.rewritten_text ?? '')}</pre></details>)}</section> : null}
          </main>
        </div>
      </section>
    </div>
