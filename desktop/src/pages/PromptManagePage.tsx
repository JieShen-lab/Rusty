  ...
  return <div className="prompt-definition-page">
    ...
    <div className="prompt-definition-layout">
      <aside className="prompt-kind-tree">
        <button className={category === 'master' ? 'active category-button' : 'category-button'} onClick={() => chooseCategory('master')} type="button">总提示词</button>
        <div className="prompt-workflow-group"><strong>工作流</strong>{(Object.entries(workflowLabels) as Array<[CreativeStrategy, string]>).map(([key, label]) => <button className={category === key ? 'active category-button nested' : 'category-button nested'} key={key} onClick={() => chooseCategory(key)} type="button">{label}</button>)}</div>
        <button className={category === 'common_task' ? 'active category-button' : 'category-button'} onClick={() => chooseCategory('common_task')} type="button">公共任务提示词</button>
      </aside>
      ...
      <main className="prompt-definition-editor">
        ...
        <footer><div><button className="button ghost" disabled={!selected} onClick={() => void duplicate()} type="button"><Copy size={15} />复制</button><button className="button ghost" disabled={!selected} onClick={() => void exportJson()} type="button"><Download size={15} />导出</button><button className="button ghost danger-quiet" disabled={!selected} onClick={() => void remove()} type="button"><Trash2 size={15} />删除</button></div><button className="button primary" disabled={busy || !form.name.trim() || (form.kind !== 'master' && !form.task_key?.trim())} onClick={() => void save()} type="button"><Save size={15} />保存</button></footer>
      </main>
    </div>
  </div>;
