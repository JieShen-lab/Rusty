...
export function PromptManagePage() {
  ...
  const grouped = useMemo(() => ({
    master: items.filter((item) => item.kind === 'master'),
    workflow: items.filter((item) => item.kind === 'workflow_task'),
    common: items.filter((item) => item.kind === 'common_task'),
  }), [items]);
  ...
  return <div className="prompt-definition-page">
    <header className="prompt-definition-header"><h1>提示词</h1><div><button className="button secondary" onClick={() => fileInput.current?.click()} type="button"><Upload size={15} />导入</button><button className="button primary" onClick={() => startNew()} type="button"><Plus size={15} />新建</button><input accept="application/json,.json" className="sr-only" ref={fileInput} onChange={(event) => void importJson(event.target.files?.[0])} type="file" /></div></header>
    ...
    <div className="prompt-definition-layout">
      <aside className="prompt-kind-tree"><PromptGroup items={grouped.master} label="总提示词" onNew={() => startNew('master')} onSelect={select} selectedId={selectedId} /><div className="prompt-workflow-group"><strong>工作流</strong>{(Object.entries(workflowLabels) as Array<[CreativeStrategy, string]>).map(([key, label]) => <PromptGroup compact items={grouped.workflow.filter((item) => item.workflow_key === key)} key={key} label={label} onNew={() => startNew('workflow_task', key)} onSelect={select} selectedId={selectedId} />)}</div><PromptGroup items={grouped.common} label="公共任务提示词" onNew={() => startNew('common_task')} onSelect={select} selectedId={selectedId} /></aside>
      <aside className="prompt-item-list"><div className="panel-heading"><h2>Prompt</h2><span>{items.length} 个</span></div>{items.map((item) => <button className={item.id === selectedId ? 'active' : ''} key={item.id} onClick={() => select(item)} type="button"><strong>{item.name}</strong><small>{describeKind(item)}</small></button>)}</aside>
      <main className="prompt-definition-editor"><div className="prompt-editor-title"><div><input aria-label="名称" placeholder="提示词名称" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><input aria-label="说明" placeholder="简短说明" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></div><label><input checked={form.is_default} onChange={(event) => setForm({ ...form, is_default: event.target.checked })} type="checkbox" />默认</label></div><div className="prompt-definition-fields"><label><span>类型</span><select value={form.kind} onChange={(event) => changeKind(event.target.value as PromptDefinitionKind, form, setForm)}><option value="master">总提示词</option><option value="workflow_task">工作流提示词</option><option value="common_task">公共任务提示词</option></select></label>{form.kind === 'workflow_task' ? <label><span>工作流</span><select value={form.workflow_key ?? 'faithful'} onChange={(event) => setForm({ ...form, workflow_key: event.target.value as CreativeStrategy })}>{Object.entries(workflowLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label> : null}{form.kind !== 'master' ? <label><span>Task key</span><input value={form.task_key ?? ''} onChange={(event) => setForm({ ...form, task_key: event.target.value })} /></label> : null}</div><label className="prompt-content-field"><span>Prompt 正文</span><textarea value={form.content} onChange={(event) => setForm({ ...form, content: event.target.value })} /></label><label className="prompt-input-field"><span>运行时大致会获得哪些输入</span><textarea value={form.input_description} onChange={(event) => setForm({ ...form, input_description: event.target.value })} /></label><footer><div><button className="button ghost" disabled={!selected} onClick={() => void duplicate()} type="button"><Copy size={15} />复制</button><button className="button ghost" disabled={!selected} onClick={() => void exportJson()} type="button"><Download size={15} />导出</button><button className="button ghost danger-quiet" disabled={!selected} onClick={() => void remove()} type="button"><Trash2 size={15} />删除</button></div><button className="button primary" disabled={busy || !form.name.trim() || (form.kind !== 'master' && !form.task_key?.trim())} onClick={() => void save()} type="button"><Save size={15} />保存</button></footer></main>
    </div>
  </div>;
}

function PromptGroup({ compact = false, items, label, onNew, onSelect, selectedId }: { compact?: boolean; items: PromptDefinition[]; label: string; onNew: () => void; onSelect: (item: PromptDefinition) => void; selectedId: number | null }) {
  return <section className={compact ? 'compact' : ''}><header><strong>{label}</strong><button aria-label={`新建${label}`} onClick={onNew} type="button"><Plus size={13} /></button></header>{items.map((item) => <button className={item.id === selectedId ? 'active' : ''} key={item.id} onClick={() => onSelect(item)} type="button">{item.name}</button>)}</section>;
}
