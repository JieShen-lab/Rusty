...
export function PromptManagePage() {
  ...
  return <div className="prompt-definition-page">
    <header className="prompt-definition-header"><h1>提示词</h1><div><button className="button secondary" onClick={() => fileInput.current?.click()} type="button"><Upload size={15} />导入</button><button className="button primary" onClick={() => startNewForCategory(category)} type="button"><Plus size={15} />新建</button><input accept="application/json,.json" className="sr-only" ref={fileInput} onChange={(event) => void importJson(event.target.files?.[0])} type="file" /></div></header>
    ...
    <div className="prompt-definition-layout">
      <aside className="prompt-kind-tree"><button className={category === 'master' ? 'active category-button' : 'category-button'} onClick={() => chooseCategory('master')} type="button">总提示词</button><div className="prompt-workflow-group"><strong>工作流</strong>{(Object.entries(workflowLabels) as Array<[CreativeStrategy, string]>).map(([key, label]) => <button className={category === key ? 'active category-button nested' : 'category-button nested'} key={key} onClick={() => chooseCategory(key)} type="button">{label}</button>)}</div><button className={category === 'common_task' ? 'active category-button' : 'category-button'} onClick={() => chooseCategory('common_task')} type="button">公共任务提示词</button></aside>
      <aside className="prompt-item-list"><div className="panel-heading"><h2>{categoryLabel(category)}</h2><span>{categoryItems.length} 个</span></div>{categoryItems.map((item) => <button className={item.id === selectedId ? 'active' : ''} key={item.id} onClick={() => select(item)} type="button"><strong>{item.name}</strong><small>{describeKind(item)}</small></button>)}</aside>
