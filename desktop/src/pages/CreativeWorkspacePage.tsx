...
function SummaryStage({ busy, value, onRun, onSave }: { busy: boolean; value: ChapterSummary | null; onRun: () => void; onSave: (value: ChapterSummary) => void }) {
  ...
  if (!draft) return <EmptyStage icon={<Sparkles size={22} />} title="先理解这一章" description="AI 会根据内容总结提示词生成剧情总结、关键事件、主要人物及人物设定。" action={<PrimaryButton disabled={busy} onClick={onRun}><Sparkles size={15} />生成内容总结</PrimaryButton>} />;
...
function DirectionStage({ busy, value, onSave }: { busy: boolean; value: ChapterCreativeIntent | null; onSave: (strategy: CreativeStrategy, instruction: string) => void }) {
  ...
  const options: Array<[CreativeStrategy, string, string]> = [['plot_adjust', '调整剧情', '根据当前章原文和要求生成旧大纲、新大纲及细节。'], ['expansion', '增加剧情', '根据整本小说原文设计插入当前章之后的新章节。'], ['plot_rewrite', '重写剧情', '根据当前章原文和要求生成重写大纲。']];
...
function AnalysisStage({ busy, instruction, value, strategy, onRun, onSave }: { busy: boolean; instruction: string; value: ChapterSpecialAnalysis | null; strategy?: CreativeStrategy; onRun: () => void; onSave: (value: ChapterSpecialAnalysis) => void }) {
  ...
  if (!draft) return <EmptyStage icon={<WandSparkles size={22} />} title="生成专项分析" description={strategyDescription(strategy)} action={<PrimaryButton disabled={busy} onClick={onRun}><Sparkles size={15} />开始分析</PrimaryButton>} />;
...
function WritingStage({ busy, workflow, onGenerate }: { busy: boolean; workflow: ChapterWorkflowState; onGenerate: (replace: boolean) => void }) {
  ...
  const description = workflow.direction?.strategy === 'plot_adjust' ? '程序会提交当前章原文、修改后的新大纲及细节和作者风格。' : '程序会提交修改后的新大纲和作者风格。';
  if (!workflow.writing) return <EmptyStage icon={<WandSparkles size={22} />} title="生成章节草稿" description={description} action={<PrimaryButton disabled={busy} onClick={() => onGenerate(false)}><Sparkles size={15} />开始写作</PrimaryButton>} />;
...
function ReviewStage({ busy, detail, workflow, onSave, onConfirm }: { busy: boolean; detail: ChapterDetail | null; workflow: ChapterWorkflowState; onSave: (text: string) => void; onConfirm: (text: string) => void }) {
  ...
  return <section className="flow-stage-card review-stage manual-review-stage"><StageHeading eyebrow="第六阶段 · 人工" title={expansion ? '新增章节审查' : '原文与修改后对照'} /><div className="manual-review-columns"><section><header><strong>{expansion ? '前一章原文' : '原始正文'}</strong><span>{detail?.chapter.word_count ?? 0} 字 · 只读</span></header><article>{detail?.chapter.original_text || '暂无原文'}</article></section><section><header><strong>{expansion ? '新增章节正文' : '修改后正文'}</strong><span>{draft.trim().length} 字 · 可编辑</span></header><textarea aria-label={expansion ? '新增章节正文' : '修改后正文'} value={draft} onChange={(event) => setDraft(event.target.value)} /></section></div><p className="manual-review-note">这里由你人工审查。系统不会再次调用模型，也不会自动判定问题。</p><footer><SecondaryButton disabled={busy || saved || !draft.trim()} onClick={() => onSave(draft)}><Save size={14} />保存修改</SecondaryButton><PrimaryButton disabled={busy || !draft.trim() || workflow.current_stage === 'confirmed'} onClick={() => onConfirm(draft)}>{workflow.current_stage === 'confirmed' ? '本章已确认' : '保存并人工确认'}<Check size={15} /></PrimaryButton></footer></section>;
}
...
function strategyDescription(value: CreativeStrategy): string { return ({ plot_adjust: '根据当前章原文生成旧大纲，以及符合调整要求的新大纲和细节。', expansion: '根据整本小说原文设计插入当前章之后的新章节大纲。', plot_rewrite: '根据当前章原文和你的要求生成重写大纲。' })[value]; }
