    ...
    def test_plot_prose_scene_anchor_plot_stays_on_one_version_state_chain(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def finish_plot(run: dict) -> dict:
                ready = plot.confirm_target_skeleton(run["id"], run["target_skeleton"])
            ...
            self.assertEqual(
                ["plot_generation", "prose_rewrite", "plot_generation"],
            ...
            self.assertIn("STYLE_MARKER", history[0]["rewritten_text"])
